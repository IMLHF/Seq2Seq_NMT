...
__all__ = ["scores_msg", "calc_ppl", "evalute"]
...
def scores_msg(scores_dict, upper_case=False):
  ...
  for metric in scores_keys:
    msg += " " + metric.upper() + ":%.4f," % scores_dict[metric]
  return msg
...
def evalute(ref_textline_file, trans_textline_file, metric, subword_option):
  """Pick a metric and evaluate depending on task."""
  # BLEU scores for translation task
  if metric.lower() == "bleu":
    evaluation_score = _bleu(ref_textline_file, trans_textline_file,
